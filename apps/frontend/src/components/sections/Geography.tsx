...
export function Geography({ count = 0, initialCities = [] }: GeographyProps) {
  ...
  return (
    <Section id="geography">
      <div className="flex flex-col gap-4 md:gap-6 lg:gap-8">
        <SectionHeader
          title={`Работаем в ${count} городах России`}
        ...
        {showLoading ? (
          <SkeletonGrid
            count={10}
            columns={5}
            CardComponent={SkeletonCityCard}
          />
        ) : showError ? (
          <ErrorMessage
          ...
          <CityGrid cities={displayCities} columns={5} />
        )}
