...
export function CTABanner({
  ...
  const { clientIosLink, clientAndroidLink, getAppLink, isLoading } =
    useClientAppLinks({
  ...
  const iosLink =
    appType === "client" ? clientIosLink : getAppLink("partner", "ios");
  const androidLink =
    appType === "client" ? clientAndroidLink : getAppLink("partner", "android");

  // Fallback URLs for client app
  const iosUrl =
    iosLink?.store_url ??
    (appType === "client" ? EXTERNAL_LINKS.APP_STORE : undefined);
  const androidUrl =
    androidLink?.store_url ??
    (appType === "client" ? EXTERNAL_LINKS.GOOGLE_PLAY : undefined);
  ...
  const handleAppClick = (platform: "ios" | "android") => {
    const source =
      analyticsSource ??
      (appType === "client" ? "cta_section" : "partner_cta_section");
      ...
      <div className="container mx-auto px-4">
        <div className="cta-banner">
          ...
          <div className="cta-banner__content">
            <h2 className="cta-banner__title">{title ?? defaultTitle}</h2>
