...
export async function generateMetadata(): Promise<Metadata> {
  const seo = await generatePageSeo("/partners/", {
    ...
    description:
      "Присоединяйтесь к сети партнёров 911. Стабильный поток заказов, удобный личный кабинет, своевременные выплаты.",
...
export default async function PartnersPage() {
  ...
  try {
    [initialAppLinks, initialContacts, initialCities, seoData] =
      await Promise.all([
  ...
  return (
    <>
      ...
      <PartnersContent
        countCity={initialCities.length}
