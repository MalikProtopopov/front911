...
export async function generateMetadata({
  ...
  try {
    ...
    const title =
      seoData?.title ||
      cityContent?.meta_title ||
      `Автопомощь в ${city.title} — эвакуатор, шиномонтаж 24/7 | 911`;
    const description =
      seoData?.meta_description ||
      cityContent?.meta_description ||
    ...
    return {
      ...
      description:
        "Шиномонтаж, эвакуатор, доставка топлива. Быстрый выезд мастера 24/7.",
    };
  }
}
...
export default async function CityPage({ params }: CityDetailPageProps) {
  ...
  const heroTitle =
    seoData?.h1_title ||
    cityContent?.h1_title ||
    (city ? `Автопомощь в ${city.title}` : "Автопомощь в городе");
    ...
    <>
      ...
      {city && !seoData?.schema_json && (
        <>
          <LocalBusinessJsonLd
            city={city}
            services={cityServices.map((s) => s.title)}
          />
          <BreadcrumbJsonLd
            items={[
      ...
      <PageLayout>
        ...
        >
          ...
          <div className="flex flex-wrap gap-6">
            ...
            {cityContent?.avg_rating &&
              parseFloat(cityContent.avg_rating) > 0 && (
                <div
                ...
                >
                  ...
                  />
                  <span>
                    {cityContent.avg_rating} ({cityContent?.review_count || 0}{" "}
                    отзывов)
                  </span>
                </div>
              )}
